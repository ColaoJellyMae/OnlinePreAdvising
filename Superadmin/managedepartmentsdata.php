...
include("../includes/config.php");
	...
	if (isset($_POST['add-department-data']))
	{
		...
		{
			echo '<script>alert("College already existed!")</script>';
			header("location:superadmin-departments.php");
		}
		else if(mysqli_num_rows($check_collegecode_run) > 0)
		{
			echo '<script>alert("College Code already existed!")</script>';
			header("location:superadmin-departments.php");
		}
		else if(file_exists("../upload/" .$_FILES["image"]["name"]))
		{
		    echo '<script>alert("College Seal already Existed")</script>';
			header("location:superadmin-departments.php");
		}
		else
		{
	...
	else if (isset($_POST['edit_college']))
	{
